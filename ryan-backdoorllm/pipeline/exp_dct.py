...
class CrossLayerDCT:
    ...
    def _exp_reweight(self, sigma: torch.Tensor, Vh: torch.Tensor) -> torch.Tensor:
        ...
        # Weight columns then re-orthonormalize via QR (fp32 required for linalg)
        V_weighted = Vh_k.T * weights.unsqueeze(0)  # (d_model, k)
        V_q, _ = torch.linalg.qr(V_weighted.float())
        return V_q.to(Vh_k.dtype).contiguous()
    ...
    def fit(
        ...
        U_rand = F.normalize(torch.randn(d_model, dim_output_projection), dim=0).to(dev).to(H_src.dtype)
        V_zeros = torch.zeros(d_model, dim_output_projection, device=dev, dtype=H_src.dtype)
        ...
        J = J_avg.get()
        _, sigma, Vh = torch.linalg.svd(J.float(), full_matrices=False)
        Vh = Vh.to(J.dtype)
        ...
        for iteration in range(1, self.n_iter + 1):
            ...
            if U_current.shape[1] < dim_output_projection:
                pad = F.normalize(
                    torch.randn(d_model, dim_output_projection - U_current.shape[1], device=dev, dtype=H_src.dtype),
                    dim=0,
                )
            ...
            V_prev = torch.zeros(d_model, dim_output_projection, device=dev, dtype=H_src.dtype)
            with torch.no_grad():
                for t in tqdm(range(0, n_train, batch_size), total=n_batches,
            ...
            _, sigma_new, Vh_new = torch.linalg.svd(J_new.float(), full_matrices=False)
            Vh_new = Vh_new.to(J_new.dtype)
